...
import 'package:redstars/src/services/repository/lookup/pricing/item_group_lookup_repository.dart';

Future<ItemGroupLookupItem> itemGroupLookupDialog(
        {String title, @required BuildContext context, @required int flag}) =>
    showDialog<ItemGroupLookupItem>(
      context: context,
      builder: (_) => ItemGroupLookupBody(title: title, flag: flag),
    );

class ItemGroupLookupBody extends BaseLookupBody {
  final String title;
  final int flag;

  ItemGroupLookupBody({this.title, this.flag});

  @override
  _ItemGroupLookupBodyState createState() => _ItemGroupLookupBodyState();
}

class _ItemGroupLookupBodyState
    extends BaseLookupBodyState<ItemGroupLookupBody, ItemGroupLookupItem> {
  @override
  Future fetchData({bool initLoad = false, String searchQuery = ""}) async {
    await ItemGroupLookUpRepository().fetchData(
        flag: widget.flag,
