...
import 'package:redstars/src/services/repository/lookup/pricing/brand_lookup_repository.dart';

Future<ItemGroupLookupItem> brandLookupDialog(
        {String title, @required BuildContext context, @required int flag}) =>
    showDialog<ItemGroupLookupItem>(
      context: context,
      builder: (_) => BrandLookupBody(title: title, flag: flag),
    );

class BrandLookupBody extends BaseLookupBody {
  final String title;
  final int flag;

  BrandLookupBody({this.title, this.flag});

  @override
  _BrandLookupBodyState createState() => _BrandLookupBodyState();
}

class _BrandLookupBodyState
    extends BaseLookupBodyState<BrandLookupBody, ItemGroupLookupItem> {
  @override
  Future fetchData({bool initLoad = false, String searchQuery = ""}) async {
    await BrandLookUpRepository().fetchData(
        flag: widget.flag,
