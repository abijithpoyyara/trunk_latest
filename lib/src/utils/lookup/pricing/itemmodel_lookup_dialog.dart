...
import 'package:redstars/src/services/repository/lookup/pricing/itemmodel_lookup_repository.dart';

Future<ItemGroupLookupItem> itemModelLookupDialog(
        {String title, @required BuildContext context, @required int flag}) =>
    showDialog<ItemGroupLookupItem>(
      context: context,
      builder: (_) => ItemModelLookupBody(title: title, flag: flag),
    );

class ItemModelLookupBody extends BaseLookupBody {
  final String title;
  final int flag;

  ItemModelLookupBody({this.title, this.flag});

  @override
  _ItemModelLookupBodyState createState() => _ItemModelLookupBodyState();
}

class _ItemModelLookupBodyState
    extends BaseLookupBodyState<ItemModelLookupBody, ItemGroupLookupItem> {
  @override
  Future fetchData({bool initLoad = false, String searchQuery = ""}) async {
    await ItemModelLookupRepository().fetchData(
        flag: widget.flag,
